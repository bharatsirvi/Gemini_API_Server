...
import videoQuickRoute from "./routes/video_quick.js";
...
app.use("/api", videoQuickRoute(ai, API_PASSWORD));
// app.use("/api", downloadRoute(ai)); // removed: route not present
...
app.use("*", (req, res) => {
  res.status(404).json({
    ...
    available_endpoints: [
      ...
      "POST /api/video-simple",
      "POST /api/video-quick",
      "GET / (simple UI)",
    ],
  });
});
